...
public class BusinessRuleServiceImpl implements BusinessRuleService
{
    ...
    @Override
    public JSONObject brCall(final String uri, String around, String action, JSONObject jsData) throws Exception
    {
        ...
        HttpEntity<String> request = new HttpEntity<>(jsData.toString(), headers);
        ...
        {
            throw new Exception(String.valueOf(response.getStatusCodeValue()).concat(":").concat(response.getBody()));
        }
        else 
        {
            try
