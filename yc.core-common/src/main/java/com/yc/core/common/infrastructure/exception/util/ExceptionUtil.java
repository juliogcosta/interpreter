...
public final class ExceptionUtil {
    ...
    public static ProblemDetails getProblemDetails(HttpServletRequest request, Exception ex) {
        return switch (ex.getClass().getSimpleName()) {
            ...
            case "IncompleteRegisterException" -> handleDataIntegrityViolation(request);
            case "DataIntegrityViolationException" -> handleDataIntegrityViolation(request);
            case "BusinessRuleConsistencyException" -> handleBusinessRuleConsistencyViolation(request);
            ...
            case "ControlledException" -> handleUnpredictableException(request);
    ...
    private static ProblemDetails handleDataIntegrityViolation(HttpServletRequest request) {
        ...
        String detail = "Violação de integridade detectada relativa à definição de dados.";

        return new ProblemDetails(title, HttpStatus.BAD_REQUEST.value(), HttpStatus.BAD_REQUEST.getReasonPhrase(),
                detail, request.getRequestURI());
    }

    private static ProblemDetails handleUnpredictableException(HttpServletRequest request) {
        String title = "Exceção lançada, não prevista.";
        String detail = "Houve uma falha interna não tratada, favor chamar suporte.";

        return new ProblemDetails(title, HttpStatus.INTERNAL_SERVER_ERROR.value(), HttpStatus.BAD_REQUEST.getReasonPhrase(),
                detail, request.getRequestURI());
    }

    private static ProblemDetails handleBusinessRuleConsistencyViolation(HttpServletRequest request) {
        String title = "Violação de integridade de regra de negócio.";
        String detail = "Violação de integridade detectada relativa ao não adequação do registro à regra de negócio.";
